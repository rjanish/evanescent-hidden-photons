...
OverlapIntegrand::OverlapIntegrand(double Rs_init, double Ls_init,
                                   ...
                                   Surface surface_init, double mass_init,
    ...
    : mode(Rs_init, Ls_init, Ki_emitter_init, omega_func_init),
      ...
      surface(surface_init), atol(atol_init), rtol(rtol_init), 
      mineval(mineval_init), maxeval(maxeval_init), 
...
double Overlap::operator()(double mass)
{
    ...
    for(auto &complex_part : re_or_im)
    {
        ...
        integrate_over_cylinderXplane(&integrand, atol, rtol, mineval, 
                                      maxeval, verbosity, result, error);
    ...
    auto omega = integrand.mode.omega_func(integrand.Rd, integrand.Ld);
    ...
    return -scale*0.5*M_PI*mass*mass*sqrt(overlap_sq); // 0.5 = 2pi/4pi 
}
